...
def undeclare_variable(rng, old_program, program_string, name_dict):
    if name_dict is not None:
        get_rev_dict(name_dict)
...
def id_mutate(rng, prog, max_num_mutations, num_mutated_progs, exact=False, name_dict=None):
    ...
    for _ in range(num_mutated_progs):
        ...
        for _ in range(num_mutations):
            ...
            try:
                mutated, this_fix, _ = undeclare_variable(rng, tokens, tokens, name_dict)
                mutation_count += 1
